...
| 2026-08-27 | Implemented WP-1 (CSV/tabular support): tabular.py (parse/classify/caps) + /inspect + /api/nlp-inspect proxy; tabular /process (text_columns override, row-indexed units, per-row VADER, prose stats nulled); /evidence units mode (row/column provenance); chunkSegments whole-row packing through EnsembleOptions.segments; CsvColumnPicker flow step; tabular-aware dashboard (banner, Responses card, sentiment label, trace Source row, row-cited evidence chips). 63 new checks (47 tabular incl. 10 fixtures, 8 chunker, 8 live-HTTP e2e); all suites + typecheck/lint/build green. Sandbox note: ~/.local user-site deps vanished between shells — re-bootstrap pip via get-pip.py when ModuleNotFoundError appears. |
| 2026-08-29 | Fireworks deployment pass: default model switched to accounts/fireworks/models/glm-5p3 everywhere (providers.ts, route fallback, ModelCompareView, legacy fireworks.ts, type docs); Fireworks adapter now surfaces the provider's own error body message (verified live: fake key -> "401: The API key you provided is invalid"); GET /api/providers?ping=<provider>&model=<id> sends a 1-token round-trip for deploy-time key/model/network verification (no key -> clean 400, unknown provider -> 400, latency reported); README Deployment section (two-process setup, NLTK bootstrap, max_tokens rationale, remote NLP_SERVICE_URL, single-user posture caveat). All suites + typecheck/lint/build green. |
