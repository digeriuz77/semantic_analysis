...
**Backlog remediation pass 2 (2026-08-26)** — completed the full review backlog except deploy-only auth (see below). **The Python service is now runtime-verified in-sandbox** (bootstrapped pip; numpy/sklearn/fastapi/nltk installed): 26 engine tests (`nlp_service/test_reliability.py`), 17 service HTTP tests (`nlp_service/test_service.py`), 8 TS ensemble tests (`nlp_service/test_ensemble.ts`), 7 COREQ repo tests (`nlp_service/test_coreq_repo.ts`) — 58 checks total, all green; `bun typecheck`/`lint`/`build` green. Remaining known items: shared-secret auth only if deployed publicly; single-linkage clustering is documented (not replaced); kappa/α are undefined for exactly-identical runs (mathematically correct, UI copy explains); compare-models route still runs per-model ensembles fully parallel with chunking (up to 6×24 calls — rate-limited to 5/5min). Next: Phase 6 (Google embeddings + vector store), Phase 7 (congruence networks).
